...
router.get('/overview', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'STATS_OVERVIEW_FAILED', message: error.message });
  }
});
...
router.get('/platform-comparison', [
  ...
  try {
    ...
    res.status(500).json({ success: false, error: 'PLATFORM_COMPARISON_FAILED', message: error.message });
  }
});

// GET /api/stats/trend/:platform — Performance trend for a specific platform
router.get('/trend/:platform', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'TREND_FETCH_FAILED', message: error.message });
  }
});
...
router.get('/monthly-report', [
  ...
  try {
    ...
    res.status(500).json({ success: false, error: 'MONTHLY_REPORT_FAILED', message: error.message });
  }
});
...
router.get('/export', [
  ...
  try {
    ...
    res.status(500).json({ success: false, error: 'EXPORT_FAILED', message: error.message });
  }
});

// GET /api/stats/failover/history — Failover event history
router.get('/failover/history', requireRole('admin'), async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'FAILOVER_STATS_FAILED', message: error.message });
  }
});
...
router.get('/realtime', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'REALTIME_STATS_FAILED', message: error.message });
  }
});
