...
// ============================================
router.post('/data-delete-request', asyncHandler(async (req, res) => {
  const userId = req.user.id;
  ...
  await auditLogger.log(req, {
    ...
    resourceType: 'user',
    resourceId: userId,
    details: { requestId, reason, regulation: 'GDPR Art.17 / PIPL 第47条' },
...
// ============================================
router.post('/confirm-delete/:requestId', asyncHandler(async (req, res) => {
...
// GET /api/v1/compliance/delete-status — 查询删除请求进度
// ============================================
router.get('/delete-status/:requestId?', asyncHandler(async (req, res) => {
  const { requestId } = req.params;
