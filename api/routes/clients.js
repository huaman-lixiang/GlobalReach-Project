...
router.post('/import', verifyToken, upload.single('file'), async (req, res) => {
  try {
    ...
    console.error('[ClientImport] Error:', error.message);
...
router.get('/export', verifyToken, async (req, res) => {
  try {
    ...
    console.error('[ClientExport] Error:', error.message);
    return res.status(500).json({
      success: false,
      error: 'EXPORT_FAILED',
...
router.get('/import-template', verifyToken, async (req, res) => {
  try {
    ...
    console.error('[ClientTemplate] Error:', error.message);
    return res.status(500).json({
      success: false,
      error: 'TEMPLATE_FAILED',
      message: error.message || '生成模板时发生错误',
    });
  }
});
